...
export function ImageUpload({
  ...
  const handleFileChange = (e) => {
    ...
    if (isSvgOnly) {
      ...
      if (!isSvgMime && !isSvgExt) {
        Notify.error("فرمت فایل غیرمجاز است. آیکون دوره باید فایل با فرمت SVG (.svg) باشد.");
        e.target.value = "";
        return;
      }
    } else {
      ...
      const hasValidExt = /\.(jpe?g|png|svg|webp)$/i.test(file.name);
    ...
    if (file.size > 5 * 1024 * 1024) {
      Notify.error("حجم فایل نباید بیشتر از ۵ مگابایت باشد.");
      e.target.value = "";
      return;
    }
  ...
  return (
    <div className={`flex flex-col gap-2 w-full text-right ${className}`}>
      ...
      <div
        className={`
          w-full ${height} border-2 border-dashed rounded-2xl flex flex-col items-center justify-center gap-3
      ...
      >
        <input
          id={id}
          type="file"
          accept={isSvgOnly ? ".svg, image/svg+xml" : accept}
        ...
        {imagePreview ? (
          <>
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white gap-1 z-5">
              ...
              <span className="text-xs font-semibold">تغییر فایل</span>
            </div>
          </>
        ) : (
          <div className="flex flex-col items-center justify-center gap-2 p-4 text-center">
            ...
            <p className="text-[11px] text-text-muted">
              {isSvgOnly ? "فقط فایل وکتور با فرمت SVG" : "JPG, PNG, SVG یا WEBP (حداکثر ۵MB)"}
            </p>
          </div>
        )}
      </div>
